...
risk_per_trade = 0.025  # Risk 2.5% of capital per trade (aggressive but controlled)
...
buy_times = []
sell_times = []
rolling_equity_high = initial_capital  # Track equity curve highs for acceleration boost
...
for idx, row in df.iterrows():
    ...
    if not regime and price > row["ema750"] * (1 + buffer) and row["ema750_slope"] > use_entry_threshold:
        ...
        # BUY immediately when regime turns on - with dynamic position sizing
        if not active_position and position == 0:
            # === CALCULATE DYNAMIC POSITION SIZE (4-Layer Professional Risk Management) ===
            
            # Layer 1: Base Risk Per Trade (volatility-adjusted)
            # Risk = 2.5% of available capital, sized by ATR to keep risk consistent
            risk_dollars = capital * risk_per_trade
            base_shares = risk_dollars / (row["atr"] * atr_multiplier)
            
            # Layer 2: Regime Strength Multiplier (size up/down based on signal strength)
            slope_mult = 1.0
            if row["ema750_slope"] > strong_slope_threshold:
                slope_mult = regime_multiplier_strong
                print(f"     [Strong] slope ({row['ema750_slope']:.6f}): +40% sizing")
            elif row["ema750_slope"] < weak_slope_threshold:
                slope_mult = regime_multiplier_weak
                print(f"     [Weak] slope ({row['ema750_slope']:.6f}): -30% sizing")
            
            position_shares = base_shares * slope_mult
            
            # Layer 3: Equity Curve Acceleration (lean in on momentum)
            current_equity = capital + (position * price if position > 0 else 0)
            if current_equity > rolling_equity_high:
                rolling_equity_high = current_equity
                position_shares *= equity_acceleration_boost
                print(f"     [Acceleration] New equity high: +20% sizing")
            
            # Layer 4: Drawdown Throttle (protect against death spiral)
            if len(equity_curve) > 0:
                current_drawdown = (current_equity - rolling_equity_high) / rolling_equity_high if rolling_equity_high > 0 else 0
                if current_drawdown < drawdown_danger_threshold:
                    position_shares *= drawdown_pause_factor
                    print(f"     [Throttle] Drawdown ({current_drawdown*100:.1f}%): -40% sizing")
            
            # Cap position to not exceed available capital (fail-safe)
            max_shares_available = capital / price
            position_shares = min(position_shares, max_shares_available * 0.9)  # Never use >90% of available capital
            
            # Execute trade
            shares = position_shares
            position = shares
            capital -= shares * price  # Reduce capital by position value
            entry_price = price
            stop_loss = entry_price - atr_multiplier * row["atr"]
            active_position = True
            active_trades += 1
            buy_times.append((timestamp, price))
            print(f"   [REGIME BUY] {timestamp}: price={price:.2f}, Shares={shares:.2f}, Position Value=${shares*price:.2f}")
    ...
    if regime and price < row["ema750"] * (1 - buffer) and row["ema750_slope"] < use_exit_threshold and strategy_mode != "HOLD":
        ...
        if active_position and position > 0:
            ...
            capital += position * price  # ADD position proceeds back to cash
            position = 0
            entry_price = 0
            stop_loss = 0
            active_position = False
    ...
    # Track equity and rolling high
    total_equity = capital + (position * price if position > 0 else 0)
    equity_curve.append(total_equity)
    if total_equity > rolling_equity_high:
        rolling_equity_high = total_equity
